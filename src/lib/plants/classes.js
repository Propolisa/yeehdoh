...
export function growForest(
    ...
    for (let i = 0; i < count; i++) {
        ...
        setTimeout(() => {
            ...
            if ((x - center.x) ** 2 + (z - center.z) ** 2 > landRadius ** 2) {
                return;
            }
...
export class Tree extends Plant {
    constructor(scene, pos, params = {}) {
        ...
        // === Environment ===
        // const humidity = scene.getAridity(pos.x, pos.z) || 0.5;
        const humidity = 0.5;
        ...
        const baseColor = Color3.FromHexString(
            COLORS[
                PALETTES[paletteType][
                    Math.floor(rand() * PALETTES[paletteType].length)
                ]
            ],
        );
        ...
        const envFactor = Scalar.Clamp(
            (fertility + moisture * 0.8) * 0.6 + (1 - humidity) * 0.4,
            0,
            1,
        );
        ...
        const leanAmt = Scalar.Lerp(
            0.02,
            0.25,
            Scalar.InverseLerp(4.5, 12.0, h),
        );
        const leanDir = new Vector3((rand() - 0.5) * 2, 0, (rand() - 0.5) * 2)
            .normalize();
        ...
        const bark = new Color4(
            0.35 + 0.15 * (1 - humidity),
            0.25 + 0.1 * (1 - humidity),
            0.15 + 0.2 * humidity,
            1,
        );
        ...
        const leafTemplate = MeshBuilder.CreateSphere("leafInstance", {
            diameter: 0.25,
            segments: 1,
        }, scene);
        ...
        sps.addShape(leafTemplate, canopyDensity, {
            positionFunction: (p) => {
                ...
                const jitter = 0.12;
                p.color = new Color4(
                    Scalar.Clamp(baseColor.r + (rand() - 0.5) * jitter, 0, 1),
                    Scalar.Clamp(baseColor.g + (rand() - 0.5) * jitter, 0, 1),
                    Scalar.Clamp(baseColor.b + (rand() - 0.5) * jitter, 0, 1),
                    1,
                );
            },
        });
        ...
        // === Animate (1 s total) ===
        [trunk, canopyParent, ...fruits].forEach((m) =>
            setTimeout(() => A.popScale(m, 20, 0, "easeOutBack"), rand() * 250)
        );
...
export class Palm extends Plant {
    constructor(scene, pos, params = {}) {
        ...
        // --- environment inputs ---
        const altitude = Scalar.Clamp(
            params.altitude ?? scene.getAltitude?.(pos.x, pos.z) ?? 0.3,
            0,
            1,
        );
        ...
        const h = Scalar.Lerp(20, 3, altitude * 0.6 + Math.random() * 0.4) *
            (0.8 + Math.random() * 0.4);

        // base–top radius relationship: thin to thick
        const baseRadius = Scalar.Lerp(0.9, 0.35, altitude) *
            (0.6 + Math.random() * 0.8); // 0.35–1.0 roughly
        const tipRadius = baseRadius * Scalar.Lerp(0.25, 0.55, Math.random());

        // curvature & lean stronger for tall palms
        const segs = 3;
        const leanMag = Scalar.Lerp(0.55, 0.1, altitude) *
            Scalar.Lerp(1.0, 1.8, h / 11);
        ...
        for (let i = 0; i < frondCount; i++) {
            ...
            for (let k = 0; k < vCount; k++) {
                fColors.push(baseTint.r, baseTint.g, baseTint.b, baseTint.a);
            }
        ...
        const fertility = params.fertility ?? 0.5;
        const moisture = params.moisture ?? 0.5;
        const fruitChance = Scalar.Clamp(
            0.2 + 0.5 * fertility + 0.4 * moisture + Math.random() * 0.2,
            0,
            1,
        );
        ...
        if (Math.random() < fruitChance) {
            const fruitMat =
                W.fruits[Math.floor(Math.random() * W.fruits.length)];
            ...
            const clusterCount = type === "coconut"
                ? 1
                : 2 + Math.floor(Math.random() * 2);

            for (let c = 0; c < clusterCount; c++) {
                const baseAng = Math.random() * Math.PI * 2;
                const hangDir = new Vector3(
                    Math.cos(baseAng),
                    -0.6,
                    Math.sin(baseAng),
                ).normalize();
                ...
                fruitSPS.addShape(fruitBaseMesh, count, {
                    positionFunction: (p) => {
                        ...
                        const col = new Color4(
                            Scalar.Clamp(
                                base.r + (Math.random() - 0.5) * cJ,
                                0,
                                1,
                            ),
                            Scalar.Clamp(
                                base.g + (Math.random() - 0.5) * cJ,
                                0,
                                1,
                            ),
                            Scalar.Clamp(
                                base.b + (Math.random() - 0.5) * cJ,
                                0,
                                1,
                            ),
                            1,
...
export class Bush extends Plant {
    constructor(scene, pos, params = {}) {
        ...
        for (let i = 0; i < n; i++) {
            ...
            const sph = this.add(
                MeshBuilder.CreateSphere("clump", {
                    diameter: diam,
                    segments: 5,
                }, scene),
                MAT.leaf,
        ...
        if (rand() > 0.5) {
            ...
            for (let i = 0; i < petalsCount; i++) {
                const p = this.add(
                    emitPetal(scene, MAT.flower, 0.25, 0.12, 0.6),
                    MAT.flower,
                );
        ...
        all.forEach((m) => {
            ...
            A.popScale(
                ...
                20,
                delay,
                rand() < 0.5 ? "easeOutBack" : "easeOutElastic",
            );
        });
...
export class Flower extends Plant {
    constructor(scene, pos, params = {}) {
        ...
        const stem = this.add(
            MeshBuilder.CreateCylinder("stem", {
                height: stemH,
                diameter: 0.05,
            }, scene),
            MAT.trunk,
        ...
        const makePetalLayer = (count, radius, verticalOffset = 0) => {
            ...
            for (let i = 0; i < count; i++) {
                ...
                const petal = this.add(
                    emitPetal(scene, MAT.flower, len, wid, curve),
                    MAT.flower,
                );
                petal.parent = head;
                ...
                petal.setVerticesData(
                    "color",
                    new Array(petal.getTotalVertices() * 4).fill(0).map(
                        (_, idx) => {
                            const m = idx % 4;
                            return m === 0
                                ? c.r
        ...
        const inner = rand() < 0.25
            ? makePetalLayer(baseCount - 1, 0.1, 0.01)
            : [];

        // === Core (fruit) ===
        const core = this.add(emitFruit(scene, MAT.fruit, 0.08), MAT.fruit);
        core.parent = head;
        const coreColor = Color3.Lerp(
            baseColor,
            new Color3(1, 0.95, 0.3),
            Scalar.Lerp(0.4, 0.7, fertility),
        );
        ...
        petalsAll.forEach((p) =>
            A.popScale(
                ...
                20,
                rand() * 300,
                rand() < 0.5 ? "easeOutElastic" : "easeOutBack",
            )
        );
...
export class FlowerCluster extends Plant {
    constructor(scene, pos, params = {}) {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const ray = new Ray(
                new Vector3(testPos.x, 50, testPos.z),
                Vector3.Down(),
                100,
            );
            const pick = scene.pickWithRay(ray, (m) => m === groundMesh);
    ...
    animateProperty(
        ...
        for (let f = 0; f <= duration; f++) {
            ...
            if (typeof from === "number") {
                keys.push({ frame: f, value: from + (to - from) * e });
            } else {
                keys.push({
                    frame: f,
                    value: new Vector3(
        ...
        window.GLOBAL_CACHE.scene.beginDirectAnimation(
            target,
            [anim],
            0,
            duration,
