...
async function main() {
  ...
  const result = await madge(entryFile, { baseDir: libDir });
  ...
  const localFiles = ordered
    .filter(f => !f.includes("node_modules"))
    .map(f => path.resolve(libDir, f));

  console.log("🧠 Files to include (in dependency order):");
  localFiles.forEach((f, i) =>
    console.log(`   ${i + 1}. ${path.relative(libDir, f).replace(/\\/g, "/")}`)
  );
  console.log("\n──────────────────────────────────────────────\n");
  ...
  const ps = spawnSync(
    ...
    [
      "-NoProfile",
      "-Command",
      `Get-Content -Raw -Encoding Unicode '${tmpFile}' | Set-Clipboard; Write-Host '✅ Clipboard write succeeded'`
    ],
    { encoding: "utf8" }
  );
